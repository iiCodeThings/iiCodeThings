...
| 单元 | 职责 | 依赖 |
|------|------|------|
| `auth` | 登录 Cookie、改密、默认用户种子 | `users` 表 |
| `models` | 端点 CRUD（列表掩码 api_key） | `llm_models` 表 |
| `sessions` | 列表/增删改、标题规则 | `sessions` 表 |
| `messages` | 分页、发送、上下文组装、SSE、入库 | sessions、attachments、上游 HTTP |
| `attachments` | 存盘、类型校验、抽文本 | 本地磁盘、antiword、python-docx |
| `search` | 分词 AND + LIKE | sessions + messages |
| `frontend` | 登录、三栏布局、流式渲染、无限滚动 | 上述 API |

各单元通过 HTTP JSON / SSE 交互；上游只允许 OpenAI 兼容的 `chat.completions` 一种协议。
